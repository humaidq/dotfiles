...
- **Nothing household-identifying enters git.** No MAC address, device name, person's name, IP from the LAN, or real domain from the household appears in any committed file, test fixture, or commit message. Fixtures use `aa:bb:cc:dd:ee:01` for MACs, `example.com` for domains, and **documentation ranges only** for addresses: `198.51.100.0/24` (TEST-NET-2) for the LAN side, `192.0.2.0/24` (TEST-NET-1) for remote peers. **No private range — 10/8, 172.16/12, 192.168/16 — appears in any fixture**, even one that does not match this network, because a reader cannot tell the difference and the next fixture copied from it might.
