{ config, lib, ... }:
let
  cfg = config.sifr.personal.ntp;
in
{
  options.sifr.personal.ntp = {
    zone = lib.mkOption {
      type = lib.types.str;
      default = "asia";
  ...
  config = {
    ...
    services.chrony = {
      ...
      extraConfig = ''
        ...
        #pool time.cloudflare.com iburst xleave ${if cfg.useNTS then "nts" else ""}
        # oreamnos
        server 10.10.0.12 iburst xleave
