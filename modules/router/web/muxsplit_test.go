...
func TestMeshListenerServesStatusAndPeers(t *testing.T) {
	...
	mesh := meshMux(pageData{}, indexTmpl, nil, nil, nil, peers, navSource{})
...
func TestLANStatusPageOffersNoPeersLink(t *testing.T) {
	...
	rec := httptest.NewRecorder()
	landingMux(pageData{}, tmpl, nil, nil, nil, navSource{}).ServeHTTP(rec, httptest.NewRequest(http.MethodGet, "/", nil))
	if strings.Contains(rec.Body.String(), `href="/peers"`) {
...
func TestMeshStatusRoutesIncludeUplink(t *testing.T) {
	...
	mesh := meshMux(pageData{}, indexTmpl, service, nil, nil, testPeersServer(t), navSource{})
